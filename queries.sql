...
-- Select employees who earn more than 40k
SELECT * 
FROM employees
WHERE salary > 40000;

-- Select all employeess who earn more than 40k AND work in Common Grounds coffeeshop
SELECT *
FROM employees
WHERE salary > 40000 AND coffeeshop_id = 1;

-- Select all employees who earn more than 40k OR work in Common Grounds
SELECT *
FROM employees
WHERE salary > 40000 OR coffeeshop_id = 1;

-- Select all employeess who earn more than 40k, work in Common Grounds coffeeshop and are female
SELECT * 
FROM employees
WHERE salary > 40000 AND coffeeshop_id = 1 AND gender = 'F';
...
SELECT CONCAT(first_name, ' ' , last_name, ' makes ', salary) AS salary
FROM employees;
...
-- replace 0s and 1s with 'True' and 'False' using CASE - WHEN function
SELECT CONCAT(first_name, ' ' , last_name) AS full_name,
CASE
   WHEN salary < 50000 THEN 'True'
   ELSE 'False'
END AS 'less than 50k'
FROM employees;
...
SELECT CONCAT(first_name, ' ' , last_name) AS full_name,
CASE 
    WHEN salary < 50000 AND gender = 'F' THEN 'True'
    ELSE 'False'
...
-- if email has '.gov', return true, otherwise false
SELECT
	email,
...
-- CASE | CASE with GROUP BY | CASE for transposing data
...
-- if between 20k-50k, then medium pay
...
-- CASE & GROUP BY 
-- Return the count of employees in each pay category (w subquery)
SELECT t.pay_category, COUNT(*)
FROM(
	SELECT
		employee_id,
	    CONCAT(first_name, ' ' , last_name) AS full_name,
...
-- OUTER JOIN (n/a in MySQL. Combined LEFT & RIGHT JOIN using UNION operator)
SELECT
  s.coffeeshop_name,
  l.city,
  l.country
...
-- Delete the values created for the JOIN exercises
